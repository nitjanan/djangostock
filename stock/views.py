...
from stock.serializers import SignUpSerializer, PurchaseOrderSerializer, PurchaseOrderItemSerializer, BaseCarSerializer, UserCarDepartmentSerializer, BaseCarDepartmentSerializer, BaseCarTypeSerializer, BaseRepairTypeSerializer, ExpBaseCarSerializer
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def apiOverviewDashboardAPP(request):
    api_urls = {
        'Api Overview Dashboard App':'/dashboard/app/api/',

        'GET Exp BaseCarType':'/exp/basecartype/api/',
        'GET Exp BaseRepairType':'/exp/baserepairtype/api/',
        'GET Exp BaseCarDepartment':'/exp/basecardepartment/api/',
        'GET Exp BaseCar':'/exp/basecar/api/',
        'GET Exp Car By Department And Branch Company All':'/all/car/department/api/between/<str:start_date>/<str:end_date>/',
        'GET Exp Repair All':'/all/exp/repair/api/between/<str:start_date>/<str:end_date>/',
        'GET Exp Work By Month All':'/all/exp/by/month/api/between/<str:start_date>/<str:end_date>/',
        'GET Exp Oil By Month All':'/all/exp/oil/by/month/api/between/<str:start_date>/<str:end_date>/',
    }
    return Response(api_urls)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def apiExpBaseCarType(request):
    queryset = BaseCarType.objects.all()
    paginator = SmallResultsSetPagination()
    result_page = paginator.paginate_queryset(queryset, request)
    serializer = BaseCarTypeSerializer(result_page, many=True)
    return paginator.get_paginated_response(serializer.data)

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def apiExpBaseRepairType(request):
    queryset = BaseCarType.objects.all()
    paginator = SmallResultsSetPagination()
    result_page = paginator.paginate_queryset(queryset, request)
    serializer = BaseRepairTypeSerializer(result_page, many=True)
    return paginator.get_paginated_response(serializer.data)

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def apiExpBaseCarDepartment(request):
    queryset = BaseCarDepartment.objects.all()
    paginator = SmallResultsSetPagination()
    result_page = paginator.paginate_queryset(queryset, request)
    serializer = BaseCarDepartmentSerializer(result_page, many=True)
    return paginator.get_paginated_response(serializer.data)
...
###############################################################################
############ ดึงชั่วโมงการทำงานของรถ ตาม car_dep และ company ทั้งหมด ##############
##############################################################################
@api_view(["GET"])
@permission_classes([IsAuthenticated])
