from stock.models import PurchaseOrder, PurchaseOrderItem, BaseCar, UserCarDepartment, BaseCarDepartment, BaseCarType, BaseRepairType
...
class UserCarDepartmentSerializer(serializers.ModelSerializer):
    driver_nam = serializers.CharField(source='user', read_only=True)
    car_code = serializers.CharField(source='car.code', read_only=True)
    code_nam = serializers.CharField(source='car.name', read_only=True)
    comp_nam = serializers.CharField(source='in_comp.affiliated.name_th', read_only=True)
...
class BaseCarTypeSerializer(serializers.ModelSerializer):

    class Meta:
        model = BaseCarType
        fields = ('id', 'name')

class BaseRepairTypeSerializer(serializers.ModelSerializer):

    class Meta:
        model = BaseRepairType
        fields = ('id', 'name')

class ExpBaseCarSerializer(serializers.ModelSerializer):

    class Meta:
        model = BaseCar
        fields = ('id', 'name', 'car_dep', 'car_type')